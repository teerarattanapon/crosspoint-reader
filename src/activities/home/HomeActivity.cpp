...
void HomeActivity::loop() {
  ...
  const bool lyra3 = SETTINGS.uiTheme == CrossPointSettings::UI_THEME::LYRA_3_COVERS;
  const int recentCount = static_cast<int>(recentBooks.size());

  if (lyra3 && recentCount > 0) {
    if (selectorIndex < recentCount) {
      lyraCarouselFocus = selectorIndex;
      ...
      buttonNavigator.onPressAndContinuous({MappedInputManager::Button::Up}, [this, recentCount, menuCount] {
        if (selectorIndex == recentCount) {
          selectorIndex = lyraCarouselFocus;
        } else {
...
void HomeActivity::render(RenderLock&&) {
  ...
  const int carouselAux =
      (lyra3 && !recentBooks.empty() && selectorIndex >= static_cast<int>(recentBooks.size())) ? lyraCarouselFocus : -1;
